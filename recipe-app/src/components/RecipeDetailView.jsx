...
const RecipeDetailView = () => {
  ...
  const instructions = meal.strInstructions ? meal.strInstructions.split(".").map
    (step => step.trim()).filter(step => step.length > 0) : [];

  return (
    <>
      <main className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Link to="/" className="text-yellow-400 hover:text-yellow-300 flex items-center mb-6 font-medium text-lg group">
          <ChevronLeft className="w-6 h-6 mr-1 transition" /> Back to Dashboard
        </Link>
        <section className='bg-gray-900 p-6 md:p-12 rounded-3xl shadow-2xl shadow-black/70 border border-gray-800'>
          <div className='lg:flex lg:space-x-12'>
            <div className='lg:w-1/2 mb-8 lg:mb-0'>
              <h1 className='text-4xl font-black text-gray-100 mb-6 leading-tight hidden lg:block'>{meal?.strMeal}</h1>

              <img src={meal?.strMealThumb} alt={meal?.strMeal} className='w-[400px] h-[400px] rounded-xl shadow-2xl shadow-black/50 object-cover border-4 border-gray-800 ring-2 ring-blue-500/50' />
            </div>
          ...
          <section className='mt-14 pt-8 border-t border-gray-800'>
            ...
            <ol className='space-y-6 list-none text-gray-300'>
              {instructions.map((step, index) => (
                <li key={index} className='text-lg leading-relaxed bg-gray-800 p-5 rounded-xl border-1-6 border-blue-500 shadow-lg shadow-black-30 transition duration-300  hover:bg-gray-700/50'>
                  <span className='font-extrabold text-yellow-400 text-xl'>{index + 1}. </span>
                  {step.trim()}
                </li>
              ))}
            </ol>
